...
func (c *HostCircuit) Define(gapi frontend.API) error {
	...
	// Debug: Check if InputOutputCommitment is set before processing
	isDryRun := false
	...
	if ctxVal, ok := dryRunContexts.Load(c); ok {
		...
		ctx.mu.Unlock()
		// In dry run mode, update Input.OutputCommitment dynamically
		c.Input.OutputCommitment = outputCommit
	}

	// Assert that the computed output matches the expected output
	// In dry run mode, Input.OutputCommitment is set above
	// In proof generation mode, Input.OutputCommitment should already be set from BuildCircuitInputStage2
